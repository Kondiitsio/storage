...
export default function StoreItems() {
  ...
  function fetchItems(store) {
    fetch(`${process.env.REACT_APP_API_URL}/store/${store.id}`)
  ...
  useEffect(() => {
    ...
    fetch(`${process.env.REACT_APP_API_URL}/store`)
      ...
      .then(data => {
        setStores(data);
        // Find the store with the matching name
        const store = data.find(store => store.name === storeName);
  
        if (store) {
          // Fetch the items for the store
          fetchItems(store);
        } else {
          setItems([]);
          setStore(null);
    ...
    <div className="px-4 sm:px-6 lg:px-8">
      ...
      <div className="sm:flex sm:items-center">
        ...
        <div className="mt-4 sm:ml-16 sm:mt-0 sm:flex-none">
          <AddItem storeId={store ? store.id : null} onAddItem={fetchItems} store={store} />
        </div>
      </div>
            ...
            <div className="relative">
              ...
              {selectedItems.length > 0 && (
                <div className="absolute top-0 flex items-center h-12 space-x-3 bg-white left-14 sm:left-12">
                  <MoveItems itemIdsToMove={itemIdsToMove} store={store} stores={stores} onMoveItems={() => fetchItems(store)} clearSelectedItems={clearSelectedItems} />
                  <button
                    type="button"
                  ...
                  {paginatedItems.map((item) => {
                    ...
                    <tr key={item.id} className={selectedItems.includes(item) ? 'bg-gray-50' : undefined}>
                      ...
                      <td className="py-4 pl-3 pr-4 text-sm font-medium text-right whitespace-nowrap sm:pr-3">
                        <EditItem item={item} storeName={store.name} onEditItem={fetchItems} store={store} />
                      </td>
                    </tr>
                    );
                  })}
